...
{
    public partial class FrmInTemPhu : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void barImport_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                if (openFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    matchList = Im.ConvertToDataTable("INTEMPHU_N", openFileDialog1.FileName, ref message) as List<IntemPhuEntity>;
                    if (string.IsNullOrEmpty(message))
                    {

                        ctrl.LoadReport(matchList, "INTEMPHU_N", this.Name, printControl1, Temp);
                    }
                    else
                    {
        ...
        private void barDowload_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                op.FileName = "In tem phu Nhua";
                if (op.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    string Temp = barMau.EditValue.ToString();
                    string remoteUri = "";
                    remoteUri = Application.StartupPath + "\\Template\\TemplateExcelInTemPhu_Nhua.xls";
        ...
        private void barDesign_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                design.ShowDialog();
                ctrl.LoadReport(matchList, "INTEMPHU_N", this.Name, printControl1, barMau.EditValue.ToString());
            }
            catch (Exception ex)
            {
                LogHelper.WriteLog(ex);
